...
export function isRecordOf<
  Keys extends (keyof keyof Record<any, unknown>)[]
>(
  thing: unknown,
...
export function isArrayOf<
  T extends (...x: any[]) => boolean
>(thing: unknown, validator: T): thing is Array<PredicateType<T>> {
  return (
    Array.isArray(thing)
    && thing.every(validator)
  )
}
 ...
 */
export type PredicateType<T extends (...x: any[]) => boolean> = T extends (target: any, ...rest: any[]) => target is infer P ? P : never
