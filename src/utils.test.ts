...
describe('utils', () => {
	...
	describe('PredicateType', () => {
		it('returns a basic predicate', () => {
			const predicateFn = (thing: unknown): thing is boolean => typeof thing === 'boolean'
			...
			expect(
				expectType<Results>(true)
			).toEqual(expectType(null))
		})
		it('returns a complex predicate', () => {
			const predicateFn = (thing: { type: string }): thing is typeof thing & { payload: unknown } => 'payload' in thing
			type Results = TypeEqual<
				PredicateType<typeof predicateFn>,
		...
		it('returns predicate even if more arguments are supplied', () => {
			const predicateFn = (thing: unknown, _other: any): thing is boolean => typeof thing === 'boolean'
		...
		it('returns never if predicate is not based on the first argument', () => {
			const predicateFn = (_thing: unknown, other: unknown): other is boolean => typeof other === 'boolean'
			type Results = TypeEqual<
				PredicateType<typeof predicateFn>,
				never
			>
